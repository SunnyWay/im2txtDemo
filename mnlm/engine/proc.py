...
def process():
    ...
    """
    ##################################
    train_captions = '/home/p-what/serverCode/im2txtDemo/mnlm/engine/iaprtc12/train_captions.txt'
    train_images = '/home/p-what/serverCode/im2txtDemo/mnlm/engine/iaprtc12/train_hidden7.txt'
    test_captions = '/home/p-what/serverCode/im2txtDemo/mnlm/engine/iaprtc12/test_captions.txt'
    test_images = '/home/p-what/serverCode/im2txtDemo/mnlm/engine/iaprtc12/test_hidden7.txt'
    context = 5
    ##################################
...
def tokenize(X, context=5, start='<start>', end='<end>'):
    ...
    tokens = [ [is_number(w) for w in x] for x in tokens ] # replace numbers by '<#>'
    for i, x in enumerate(tokens):
        tokens[i] = [start] * context + x + [end] # add start tag in the beginning and add end tag in the end
    return tokens
...
def compute_labels(ngrams, word_dict, context=5):
    ...
    count = 0
    train_dict = {}
    for w in uniq_ngrams:   # index all n-1 tuples, output a dict named train_dict
        train_dict[w] = count
        count = count + 1
